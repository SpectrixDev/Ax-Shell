    ...
    def on_reset(self, widget):
        ...
        if dialog.run() == Gtk.ResponseType.YES:
            ...
            self.ws_num_switch.set_active(
                get_bind_var("bar_workspace_show_number")
            )
            ...
            for name, switch in self.component_switches.items():
                switch.set_active(
                    get_bind_var(f"bar_{name}_visible")
                )
            self.corners_switch.set_active(
                get_bind_var("corners_visible")
            )
